...
export async function rotatePdf(
  ...
  const targets = ranges.trim()
    ? parseRanges(ranges, pages.length)
    : pages.map((_, i) => i);
...
export async function readPdfMeta(
  file: File,
): Promise<{ meta: PdfMeta; pageCount: number }> {
  const doc = await PDFDocument.load(await file.arrayBuffer(), {
    ignoreEncryption: true,
...
export async function textToPdf(text: string, title: string): Promise<Uint8Array> {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (h) {
      ...
      for (const l of wrapLine(h[2], bold, s))
        outLines.push({ text: l, bold: true, size: s });
  ...
  let y = pageH - margin;
  const cursor = rgb(0.11, 0.11, 0.12);
  if (title) {
    doc.setTitle(title);
    doc.setCreator("Documender");
    doc.setProducer("Documender");
  }
